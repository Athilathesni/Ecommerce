...
const SellerPage = () => {
  ...
  useEffect(() => {
    const fetchCompanyDetails = async () => {
      if (token) {
      try {
        const res = await axios.get("http://localhost:3004/api/getCompany", {
    ...
    const fetchProductCounts = async () => {
      try {
        const res = await axios.get("http://localhost:3004/api/getProducts", {
          headers: { Authorization: `Bearer ${token}` },
        });
  ...
  const handleSave = async () => {
    try {
      const res = await axios.post("http://localhost:3004/api/addCompany", formData,
        { headers: { Authorization: `Bearer ${token}` } }
      );
