...
const ProfileInfo = () => {
  ...
  useEffect(() => {
    const fetchUserDetails = async () => {
      try {
        const res = await axios.get("http://localhost:3004/api/getuserData", {
  ...
  const handleSave = async () => {
    try {
      const res = await axios.put("http://localhost:3004/api/updateUser", formData, {
        headers: { Authorization: `Bearer ${token}` },
      });
      if (res.status === 200) {
  ...
  const handleAddAddress = async () => {
    try {
      const res = await axios.post("http://localhost:3004/api/addAddress", address, {
        headers: { Authorization: `Bearer ${token}` },
      });
      if (res.status === 201) {
